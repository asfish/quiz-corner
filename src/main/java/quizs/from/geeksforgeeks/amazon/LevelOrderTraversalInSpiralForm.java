 ...
 *                  7  6  5  4
 *
 * We can print spiral order traversal in O(n) time and O(n) extra space. The idea
 * is to use two stacks. We can use one stack for printing from left to right and
 * other stack for printing from right to left. In every iteration, we have nodes
 * of one level in one of the stacks. We print the nodes, and push nodes of next
